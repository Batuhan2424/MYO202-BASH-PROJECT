...
if [ "$OS_TYPE" == "Darwin" ]; then
    ...
    wmic csproduct get UUID /value 2>/dev/null | tr -d '\r' | grep = >> "$LOG_FILE"
    wmic diskdrive get Model,SerialNumber /value 2>/dev/null | tr -d '\r' | grep = >> "$LOG_FILE"
    getmac /v /fo list 2>/dev/null | tr -d '\r' | grep -E "Fiziksel|Physical|Yol" >> "$LOG_FILE"
fi
